...
_Static_assert(OP_MIX == 56, "LABC v2 MIX value changed");
_Static_assert(OP_MAP == 57, "LABC v2 MAP value changed");
_Static_assert(OP_SUPPORT == 58, "LABC v2 SUPPORT value changed");
_Static_assert(OP_EXPECT == 59, "LABC v2 EXPECT value changed");
_Static_assert(OP_VALIDATE == 60, "LABC v2 VALIDATE value changed");
_Static_assert(OP_REVISION == 61, "LABC v2 REVISION value changed");
_Static_assert(OP_ATTENUATE == 62, "LABC v2 ATTENUATE value changed");
_Static_assert(OP_TRACE_DISTANCE == 63, "LABC v2 TRACE_DISTANCE value changed");
_Static_assert(OP_APPEND_REDUNDANT == 64, "LABC v2 APPEND_REDUNDANT value changed");
_Static_assert(OP_APPEND_FULL_REDUNDANCY == 65, "LABC v2 APPEND_FULL_REDUNDANCY value changed");
_Static_assert(OP_APPEND_COMPLEMENTARY == 66, "LABC v2 APPEND_COMPLEMENTARY value changed");
_Static_assert(OP_ADT_BUILD == 67, "LABC v2 ADT_BUILD value changed");
_Static_assert(OP_ADT_CASE == 68, "LABC v2 ADT_CASE value changed");
...
static int test_lazy_datasets(void) {
    LanaChunk chunk;
    LanaVM vm;
    uint32_t two, bound, index, fn_index, main_entry;
    lana_chunk_init(&chunk);
    chunk.version = LABC_VERSION;

    /* Generator function (arity 1): returns index * 2. */
    CHECK(add_number(&chunk, 2.0, &two) == 0);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LOAD_CONST, 1, 0, 0, two)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_BINARY, 0, 1, 2, LANA_BINARY_MULTIPLY)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_RETURN, 2, 0, 0, 0)) == LANA_OK);
    CHECK(lana_chunk_add_function(&chunk, "gen", 0u, 3u, 1u, &fn_index) == LANA_OK);
    CHECK(fn_index == 0u);

    /* Main program: lazy(gen, 5); force(lazy, 3) -> 6.0. */
    main_entry = (uint32_t)chunk.code_count;
    CHECK(add_number(&chunk, 5.0, &bound) == 0);
    CHECK(add_number(&chunk, 3.0, &index) == 0);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LOAD_CONST, 1, 0, 0, bound)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LAZY, 2, fn_index, 1, 0)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LOAD_CONST, 3, 0, 0, index)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_FORCE, 4, 2, 3, 0)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_HALT, 0, 0, 0, 0)) == LANA_OK);
    chunk.entry = main_entry;

    lana_vm_init(&vm, &chunk);
    CHECK(lana_vm_run(&vm) == LANA_OK);
    CHECK(vm.frames[0].registers[2].type == VAL_LAZY);
    CHECK(vm.frames[0].registers[2].as.lazy.function == 0u);
    CHECK(vm.frames[0].registers[2].as.lazy.bound == 5u);
    CHECK(vm.frames[0].registers[4].type == VAL_NUMBER);
    CHECK(vm.frames[0].registers[4].as.number == 6.0);
    lana_vm_free(&vm);
    lana_chunk_free(&chunk);

    /* lazy_bound host call returns the bound. */
    lana_chunk_init(&chunk);
    chunk.version = LABC_VERSION;
    CHECK(lana_chunk_add_function(&chunk, "gen", 0u, 1u, 1u, &fn_index) == LANA_OK);
    CHECK(add_number(&chunk, 5.0, &bound) == 0);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LOAD_CONST, 1, 0, 0, bound)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LAZY, 2, fn_index, 1, 0)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_HOST_CALL, 3, LANA_HOST_LAZY_BOUND, 2, 1)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_HALT, 0, 0, 0, 0)) == LANA_OK);
    lana_vm_init(&vm, &chunk);
    CHECK(lana_vm_run(&vm) == LANA_OK);
    ...
    /* Out-of-bounds index returns LANA_ERR_LIMIT. */
    lana_chunk_init(&chunk);
    chunk.version = LABC_VERSION;
    CHECK(lana_chunk_add_function(&chunk, "gen", 0u, 1u, 1u, &fn_index) == LANA_OK);
    CHECK(add_number(&chunk, 5.0, &bound) == 0);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LOAD_CONST, 1, 0, 0, bound)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LAZY, 2, fn_index, 1, 0)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LOAD_CONST, 3, 0, 0, bound)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_FORCE, 4, 2, 3, 0)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_HALT, 0, 0, 0, 0)) == LANA_OK);
    lana_vm_init(&vm, &chunk);
    ...
    /* Non-lazy operand returns LANA_ERR_TYPE. */
    lana_chunk_init(&chunk);
    chunk.version = LABC_VERSION;
    CHECK(add_number(&chunk, 0.0, &index) == 0);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LOAD_CONST, 2, 0, 0, index)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LOAD_CONST, 3, 0, 0, index)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_FORCE, 4, 2, 3, 0)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_HALT, 0, 0, 0, 0)) == LANA_OK);
    lana_vm_init(&vm, &chunk);
    ...
    /* Verifier accepts LAZY and FORCE. */
    lana_chunk_init(&chunk);
    chunk.version = LABC_VERSION;
    CHECK(lana_chunk_add_function(&chunk, "gen", 0u, 3u, 1u, &fn_index) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LAZY, 2, fn_index, 1, 0)) == LANA_OK);
...
static int test_correlated_host_call(void) {
    LanaChunk chunk;
    LanaVM vm;
    uint32_t p, d_re, d_im, coefficient;
    lana_chunk_init(&chunk);
    chunk.version = LABC_VERSION;

    /* correlated(x, y, 0) with x.p = y.p = 0.5 -> uniform 2x2 joint. */
    CHECK(add_number(&chunk, 0.5, &p) == 0);
    CHECK(add_number(&chunk, 0.0, &d_re) == 0);
    CHECK(add_number(&chunk, 0.0, &d_im) == 0);
    CHECK(add_number(&chunk, 0.0, &coefficient) == 0);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_STATE_NEW, 1, p, d_re, d_im)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_STATE_NEW, 2, p, d_re, d_im)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LOAD_CONST, 3, 0, 0, coefficient)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_HOST_CALL, 4, LANA_HOST_CORRELATED, 1, 3)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_HALT, 0, 0, 0, 0)) == LANA_OK);
    lana_vm_init(&vm, &chunk);
    CHECK(lana_vm_run(&vm) == LANA_OK);
    CHECK(vm.frames[0].registers[4].type == VAL_JOINT_STATE);
    CHECK(vm.frames[0].registers[4].as.joint->count == 2u);
    CHECK(vm.frames[0].registers[4].as.joint->row_count == 4u);
    CHECK(strcmp(vm.frames[0].registers[4].as.joint->names[0], "x") == 0);
    CHECK(strcmp(vm.frames[0].registers[4].as.joint->names[1], "y") == 0);
    CHECK(fabs(vm.frames[0].registers[4].as.joint->rows[0].weight - 0.25) < 1e-9);
    CHECK(fabs(vm.frames[0].registers[4].as.joint->rows[1].weight - 0.25) < 1e-9);
    CHECK(fabs(vm.frames[0].registers[4].as.joint->rows[2].weight - 0.25) < 1e-9);
    CHECK(fabs(vm.frames[0].registers[4].as.joint->rows[3].weight - 0.25) < 1e-9);
    lana_vm_free(&vm);
    lana_chunk_free(&chunk);

    /* correlated(x, y, 1) -> full correlation: p00 = p11 = 0.5, off-diagonal 0. */
    lana_chunk_init(&chunk);
    chunk.version = LABC_VERSION;
    CHECK(add_number(&chunk, 0.5, &p) == 0);
    CHECK(add_number(&chunk, 0.0, &d_re) == 0);
    CHECK(add_number(&chunk, 0.0, &d_im) == 0);
    CHECK(add_number(&chunk, 1.0, &coefficient) == 0);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_STATE_NEW, 1, p, d_re, d_im)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_STATE_NEW, 2, p, d_re, d_im)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LOAD_CONST, 3, 0, 0, coefficient)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_HOST_CALL, 4, LANA_HOST_CORRELATED, 1, 3)) == LANA_OK);
    ...
    CHECK(lana_vm_run(&vm) == LANA_OK);
    CHECK(vm.frames[0].registers[4].type == VAL_JOINT_STATE);
    CHECK(vm.frames[0].registers[4].as.joint->row_count == 2u);
    CHECK(fabs(vm.frames[0].registers[4].as.joint->rows[0].weight - 0.5) < 1e-9);
    CHECK(fabs(vm.frames[0].registers[4].as.joint->rows[1].weight - 0.5) < 1e-9);
    lana_vm_free(&vm);
    lana_chunk_free(&chunk);

    /* |d| > 1 returns LANA_ERR_INVALID_PARAMETERS. */
    lana_chunk_init(&chunk);
    chunk.version = LABC_VERSION;
    CHECK(add_number(&chunk, 0.5, &p) == 0);
    CHECK(add_number(&chunk, 0.0, &d_re) == 0);
    CHECK(add_number(&chunk, 0.0, &d_im) == 0);
    CHECK(add_number(&chunk, 2.0, &coefficient) == 0);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_STATE_NEW, 1, p, d_re, d_im)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_STATE_NEW, 2, p, d_re, d_im)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LOAD_CONST, 3, 0, 0, coefficient)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_HOST_CALL, 4, LANA_HOST_CORRELATED, 1, 3)) == LANA_OK);
    ...
    /* Non-STATE operand returns LANA_ERR_TYPE. */
    lana_chunk_init(&chunk);
    chunk.version = LABC_VERSION;
    CHECK(add_number(&chunk, 0.5, &p) == 0);
    CHECK(add_number(&chunk, 0.0, &d_re) == 0);
    CHECK(add_number(&chunk, 0.0, &d_im) == 0);
    CHECK(add_number(&chunk, 0.0, &coefficient) == 0);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LOAD_CONST, 1, 0, 0, p)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_STATE_NEW, 2, p, d_re, d_im)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_LOAD_CONST, 3, 0, 0, coefficient)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_HOST_CALL, 4, LANA_HOST_CORRELATED, 1, 3)) == LANA_OK);
    CHECK(lana_chunk_emit(&chunk, instruction(OP_HALT, 0, 0, 0, 0)) == LANA_OK);
