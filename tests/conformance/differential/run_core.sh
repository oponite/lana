#!/usr/bin/env bash
# Differential conformance spot-checks for core VM operations (Phase 3).
#
# For each fixture in core/, assemble with the C11 assembler, run under both
# the C11 `lanavm` and the Rust `lana-cli`, and assert byte-identical stdout,
# stderr, and exit codes. A second pass runs both VMs with `--stats` and
# asserts identical instruction/state-transition/opcode accounting (the
...
#   ./tests/differential/run_core.sh
#
...
FIXTURES="$REPO_ROOT/tests/conformance/differential/core"
WORK="$(mktemp -d)"
...
for fixture in "$FIXTURES"/*.lasm; do
    ...
    "$C11" run "$WORK/$name.labc" "${args[@]}" >"$WORK/$name.c11.out" 2>"$WORK/$name.c11.err"
    c11_exit=$?
    "$RUST" run "$WORK/$name.labc" "${args[@]}" >"$WORK/$name.rust.out" 2>"$WORK/$name.rust.err"
    rust_exit=$?
    ...
    "$C11" run "$WORK/$name.labc" "${args[@]}" --stats >"$WORK/$name.c11.stats.out" 2>"$WORK/$name.c11.stats.err"
    "$RUST" run "$WORK/$name.labc" "${args[@]}" --stats >"$WORK/$name.rust.stats.out" 2>"$WORK/$name.rust.stats.err"
