#!/usr/bin/env bash
# Differential conformance spot-checks for the task model (Increment 4).
#
# For each fixture in tasks/, assemble with the C11 assembler, run under both
# the C11 `lanavm` and the Rust `lana-cli`, and assert byte-identical stdout,
# stderr, and exit codes. The Rust CLI must be built first:
#
#   cargo build -p lana-cli
#   ./tests/differential/run_tasks.sh
#
...
FIXTURES="$REPO_ROOT/tests/conformance/differential/tasks"
WORK="$(mktemp -d)"
...
for fixture in "$FIXTURES"/*.lasm; do
    ...
    # Optional per-fixture extra CLI args (e.g. --max-tasks 1).
    args=()
