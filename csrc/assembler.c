...
static int host_call_id(const char *name) {
    static const char *names[] = {"args", "read_text", "write_text", "now", "random", "assert"};
    size_t index;
    for (index = 0; index < sizeof(names) / sizeof(names[0]); ++index)
        if (strcmp(name, names[index]) == 0) return (int)index;
    return -1;
...
static SSError emit_line(SSChunk *chunk, char **tokens, size_t count, uint32_t line,
                         Fixup *fixups, size_t *fixup_count, SSErrorInfo *error) {
...
#define REG(token, destination) do { if (!parse_register((token), &(destination))) { ss_error_set(error, SS_ERR_REGISTER, chunk->code_count, OP_NOP, line, "invalid register %s", (token)); return SS_ERR_REGISTER; } } while (0)
    if (strcmp(tokens[0], "NOP") == 0) { EXPECT(1); ins.opcode = OP_NOP; }
        ...
        if (id < 0) return SS_ERR_HISTORY; ins.opcode = OP_HISTORY_CONFIG; ins.a = a; ins.b = b; ins.c = (uint32_t)id;
        ...
        if (*fixup_count >= 256u || !copy_name(fixups[*fixup_count].name, tokens[label_token])) return SS_ERR_LIMIT;
        ...
        if (function_index == chunk->function_count || number < 0.0 || number > SS_MAX_REGISTERS) return SS_ERR_FORMAT;
        ...
        for (function_index = 0; function_index < chunk->function_count; ++function_index)
            if (strcmp(tokens[1], chunk->functions[function_index].name) == 0) break;
        if (function_index == chunk->function_count || number < 0.0 || number > SS_MAX_REGISTERS || floor(number) != number) return SS_ERR_FORMAT;
...
SSError ss_assemble_file(const char *path, SSChunk *chunk, SSErrorInfo *error) {
    ...
    char buffer[4096];
    uint32_t line = 0;
    Label labels[256]; size_t label_count = 0;
    Fixup fixups[256]; size_t fixup_count = 0;
    ...
    while (fgets(buffer, sizeof(buffer), file) != NULL) {
        ...
        if (cursor[length - 1u] == ':') {
            cursor[length - 1u] = '\0';
            if (label_count >= 256u || !copy_name(labels[label_count].name, trim(cursor))) { result = SS_ERR_LIMIT; break; }
        ...
        result = emit_line(chunk, tokens, count, line, fixups, &fixup_count, error);
        if (result != SS_OK) break;
    }
    (void)fclose(file);
    if (result == SS_OK) {
    ...
    if (result != SS_OK) ss_chunk_free(chunk);
    return result;
}
