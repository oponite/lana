"""Stable JSON-file subprocess boundary for Lana 2.0 programs."""
...
_VERSION_PATTERN = re.compile(r"^Lana (2\.\d+\.\d+) \(LABC v2,")


class LanaCompatibilityError(RuntimeError):
    """Raised when the selected executable is not supported Lana 2.0 with LABC v2."""
    ...
    def _check_compatibility(self) -> str:
        ...
        if completed.returncode != 0 or match is None:
            ...
            raise LanaCompatibilityError(
                f"expected Lana 2.0 with LABC v2; got: {detail}"
            )
