...
//!
//! # Host-call policy (LIP-025)
//!
//! Filesystem host calls are unavailable by default in WASM (there is no
//! ambient filesystem). A program that uses a gated host call (`read_text`,
//! `write_text`, `directory_list`, `directory_create`, `path_exists`,
//! `write_text_atomic`) fails with `LANA_ERR_UNSUPPORTED_OPERATION` unless the
//! host explicitly wires that capability. The `capabilities` argument is a JSON
//! object naming which gated host calls are wired (value `true`) plus optional
//! `seed`, `instruction_limit`, and `memory_limit` (MiB) keys:
//!
...
use lana_vm::vm::{
    LANA_HOST_DIRECTORY_CREATE, LANA_HOST_DIRECTORY_LIST, LANA_HOST_PATH_EXISTS,
    LANA_HOST_READ_TEXT, LANA_HOST_WRITE_TEXT, LANA_HOST_WRITE_TEXT_ATOMIC,
};
use lana_vm::Vm;
...
/// Host calls gated behind explicit capabilities in WASM (LIP-025). These are
/// the filesystem host calls; there is no ambient FS in a WASM embedding.
