//! Embed the self-hosted compiler bytecode (`lana-compiler.labc`) so the WASI
//! binary can compile Lana source without a filesystem — mirroring the
//! `lana-wasm` crate's `build.rs`.
//!
