...
LanaError lana_chunk_verify(const LanaChunk *chunk, LanaErrorInfo *error) {
    ...
    for (ip = 0; ip < chunk->code_count; ++ip) {
        ...
        switch ((OpCode)ins->opcode) {
            ...
            case OP_ADT_CASE:
                result = verify_register(error, ip, ins, ins->a);
                if (result == LANA_OK && !constant_valid(chunk, ins->b)) result = LANA_ERR_CONSTANT;
                if (result == LANA_OK && chunk->constants[ins->b].type != VAL_NUMBER) result = LANA_ERR_TYPE;
                if (result == LANA_OK && ins->imm >= chunk->code_count) result = LANA_ERR_JUMP;
                break;
            case OP_ADT_GET:
                result = verify_register(error, ip, ins, ins->a);
...
void lana_disassemble_instruction(const LanaChunk *chunk, size_t offset, FILE *out) {
    ...
    switch ((OpCode)ins->opcode) {
        ...
        case OP_STATE_BUILD:
            ...
            break;
        case OP_MIX:
            (void)fprintf(out, "R%u <- mix(R%u, R%u, w=R%u)", ins->a,
                          ins->b, ins->c, ins->imm);
            break;
        case OP_MAP:
            (void)fprintf(out, "R%u <- map(R%u, %s)", ins->a, ins->b,
                          ins->c == LANA_TRANSFORM_INVERT ? "invert" : "neutralize");
            break;
        case OP_SUPPORT:
            (void)fprintf(out, "R%u <- support(R%u, limit=%u)", ins->a, ins->b, ins->imm);
            break;
        case OP_EXPECT:
            (void)fprintf(out, "R%u <- expect(R%u, %s)", ins->a, ins->b,
                          ins->imm == LANA_OBSERVABLE_PROBABILITY ? "probability" : "unknown");
            break;
        case OP_VALIDATE:
            (void)fprintf(out, "R%u <- validate(R%u, R%u)", ins->a, ins->b, ins->c);
            break;
        case OP_REVISION:
            (void)fprintf(out, "R%u <- revision(R%u)", ins->a, ins->b);
            break;
        case OP_TRANSFORM:
        ...
        case OP_APPEND:
            ...
            break;
        case OP_ATTENUATE:
            (void)fprintf(out, "R%u <- attenuate(R%u, factor=R%u)", ins->a, ins->b, ins->c);
            break;
        case OP_TRACE_DISTANCE:
            (void)fprintf(out, "R%u <- trace_distance(R%u, R%u)", ins->a, ins->b, ins->c);
            break;
        case OP_APPEND_REDUNDANT:
            (void)fprintf(out, "R%u R%u redundant(strength=R%u) -> R%u", ins->a, ins->b, ins->imm, ins->c);
            break;
        case OP_APPEND_FULL_REDUNDANCY:
            (void)fprintf(out, "R%u R%u full_redundancy -> R%u", ins->a, ins->b, ins->c);
            break;
        case OP_APPEND_COMPLEMENTARY:
            (void)fprintf(out, "R%u R%u complementary(strength=R%u) -> R%u", ins->a, ins->b, ins->imm, ins->c);
            break;
        case OP_ADT_BUILD:
            (void)fprintf(out, "R%u..R%u variant=K%u -> R%u", ins->b,
                          ins->b + ins->c - 1u, ins->imm, ins->a);
            break;
        case OP_ADT_CASE:
            (void)fprintf(out, "R%u variant=K%u -> %u", ins->a, ins->b, ins->imm);
            break;
        case OP_ADT_GET:
            (void)fprintf(out, "R%u <- R%u[%u]", ins->a, ins->b, ins->c);
            break;
        case OP_LAZY:
            (void)fprintf(out, "R%u <- lazy(function[%u], bound=R%u)", ins->a, ins->b, ins->c);
            break;
        case OP_FORCE:
