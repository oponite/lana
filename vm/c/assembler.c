...
static int observable_id(const char *name) {
    if (strcmp(name, "probability") == 0) return LANA_OBSERVABLE_PROBABILITY;
    return -1;
}
...
#define REG(token, destination) do { if (!parse_register((token), &(destination))) { lana_error_set(error, LANA_ERR_REGISTER, chunk->code_count, OP_NOP, line, "invalid register %s", (token)); return LANA_ERR_REGISTER; } } while (0)
    if (strcmp(tokens[0], "NOP") == 0) { EXPECT(1); ins.opcode = OP_NOP; }
        ...
        ins.c = (uint32_t)id;
        ins.imm = 0u;
    } else if (strcmp(tokens[0], "SUPPORT") == 0) {
        EXPECT(4);
        REG(tokens[1], a);
        REG(tokens[2], b);
        if (!sample_count(tokens[3], &ins.imm)) return LANA_ERR_FORMAT;
        ins.opcode = OP_SUPPORT;
        ins.a = a;
        ins.b = b;
        ins.c = 0u;
    } else if (strcmp(tokens[0], "EXPECT") == 0) {
        EXPECT(4);
        REG(tokens[1], a);
        REG(tokens[2], b);
        id = observable_id(tokens[3]);
        if (id < 0) return LANA_ERR_UNSUPPORTED_OPERATION;
        ins.opcode = OP_EXPECT;
        ins.a = a;
        ins.b = b;
        ins.c = 0u;
        ins.imm = (uint32_t)id;
        ...
        REG(tokens[2], b);
        REG(tokens[3], c);
        ins.opcode = OP_VALIDATE;
        ins.a = a;
        ins.b = b;
        ins.c = c;
        ins.imm = 0u;
    } else if (strcmp(tokens[0], "REVISION") == 0) {
        EXPECT(3);
        REG(tokens[1], a);
        REG(tokens[2], b);
        ins.opcode = OP_REVISION;
        ins.a = a;
        ins.b = b;
        ins.c = 0u;
        ins.imm = 0u;
        ...
        ins.c = c;
        ins.imm = 0u;
    } else if (strcmp(tokens[0], "ATTENUATE") == 0) {
        EXPECT(4);
        REG(tokens[1], a);
        REG(tokens[2], b);
        REG(tokens[3], c);
        ins.opcode = OP_ATTENUATE;
        ins.a = a;
        ins.b = b;
        ins.c = c;
        ins.imm = 0u;
    } else if (strcmp(tokens[0], "TRACE_DISTANCE") == 0) {
        EXPECT(4);
        REG(tokens[1], a);
        REG(tokens[2], b);
        REG(tokens[3], c);
        ins.opcode = OP_TRACE_DISTANCE;
        ins.a = a;
        ins.b = b;
        ins.c = c;
        ins.imm = 0u;
    } else if (strcmp(tokens[0], "APPEND_REDUNDANT") == 0 ||
               strcmp(tokens[0], "APPEND_COMPLEMENTARY") == 0) {
        EXPECT(5);
        REG(tokens[1], a);
        REG(tokens[2], b);
        REG(tokens[3], c);
        REG(tokens[4], ins.imm);
        ins.opcode = strcmp(tokens[0], "APPEND_REDUNDANT") == 0
                         ? OP_APPEND_REDUNDANT : OP_APPEND_COMPLEMENTARY;
        ins.a = a;
        ins.b = b;
        ins.c = c;
    } else if (strcmp(tokens[0], "APPEND_FULL_REDUNDANCY") == 0) {
        EXPECT(4);
        REG(tokens[1], a);
        REG(tokens[2], b);
        REG(tokens[3], c);
        ins.opcode = OP_APPEND_FULL_REDUNDANCY;
        ins.a = a;
        ins.b = b;
        ins.c = c;
        ins.imm = 0u;
    } else if (strcmp(tokens[0], "ADT_BUILD") == 0) {
        EXPECT(5);
        REG(tokens[1], a);
        REG(tokens[2], b);
        number = strtod(tokens[3], NULL);
        if (number < 0 || number > LANA_MAX_REGISTERS) return LANA_ERR_LIMIT;
        result = number_constant(chunk, tokens[4], &ins.imm);
        if (result != LANA_OK) return result;
        ins.opcode = OP_ADT_BUILD;
        ins.a = a;
        ins.b = b;
        ins.c = (uint32_t)number;
    } else if (strcmp(tokens[0], "ADT_CASE") == 0) {
        EXPECT(4);
        REG(tokens[1], a);
        result = number_constant(chunk, tokens[2], &ins.b);
        if (result != LANA_OK) return result;
        if (*fixup_count >= LANA_ASSEMBLER_MAX_FIXUPS ||
            !copy_name(fixups[*fixup_count].name, tokens[3])) return LANA_ERR_LIMIT;
        fixups[*fixup_count].instruction = (uint32_t)chunk->code_count;
        ++*fixup_count;
        ins.opcode = OP_ADT_CASE;
        ins.a = a;
        ins.c = 0u;
        ins.imm = 0u;
    } else if (strcmp(tokens[0], "ADT_GET") == 0) {
        EXPECT(4);
        REG(tokens[1], a);
        REG(tokens[2], b);
        number = strtod(tokens[3], NULL);
        if (number < 0 || number > LANA_MAX_REGISTERS) return LANA_ERR_LIMIT;
        ins.opcode = OP_ADT_GET;
        ins.a = a;
        ins.b = b;
        ins.c = (uint32_t)number;
        ins.imm = 0u;
    } else if (strcmp(tokens[0], "LAZY") == 0) {
        size_t function_index;
        EXPECT(4); REG(tokens[1], a); REG(tokens[3], c);
        ...
        ins.opcode = OP_LAZY; ins.a = a; ins.b = (uint32_t)function_index; ins.c = c; ins.imm = 0u;
    } else if (strcmp(tokens[0], "FORCE") == 0) {
        EXPECT(4);
        REG(tokens[1], a);
        REG(tokens[2], b);
        REG(tokens[3], c);
